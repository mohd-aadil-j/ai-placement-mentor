...
const Register = () => {
  ...
  useEffect(() => {
    // Prevent showing register when already authenticated
    if (isAuthenticated) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await authApi.register({ name, email, password, targetRole });
  ...
  return (
    <div className="relative min-h-screen overflow-hidden flex items-center justify-center p-4">
      ...
      <div className="relative z-10 bg-slate-900/80 backdrop-blur-sm border border-slate-700 p-6 sm:p-8 rounded-lg shadow-xl w-full max-w-md">
        <h2 className="text-3xl sm:text-4xl font-bold text-center mb-6 text-blue-400">REGISTER</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-6">
            <label className="block text-slate-300 text-sm font-bold mb-2" htmlFor="targetRole">
        ...
        <p className="text-center mt-4 text-slate-400">
          ...
          <Link to="/login" className="text-blue-400 hover:text-blue-300 underline transition">
            Login here
          </Link>
        </p>
      </div>
